...
#define CONF_NCOLOURS (22 + 2)               /* 16 + 6 special ones + 2 custom ones */
#define OSCP_NCOLOURS (22 + 2)               /* same as CONF, but different order */
#define OSC4_NCOLOURS (262 + 2)              /* 256 + the same 6 special ones + the same 2 custom ones */
...
#define CONF_COLOUR_LIST(X)                     \
    ...
    X(cursor_fg_ime, "Cursor Text(IME ON)")     \
    X(cursor_bg_ime, "Cursor Colour(IME ON)")   \
    /* end of list */
...
#define OSCP_COLOUR_LIST(X)                     \
    ...
    X(cursor_fg)                                \
    X(cursor_bg)                                \
    /* additional colors for custom build */    \
    X(cursor_fg_ime)                            \
    X(cursor_bg_ime)                            \
    /* end of list */
...
#define CONFIG_OPTIONS(X) \
    ...
    X(BOOL, NONE, alt_metabit)                 /* set meta instead of escape */ \
    X(BOOL, NONE, ctrl_tab_switch)             /* switch PuTTY windows with Ctrl+Tab */ \
    X(BOOL, NONE, switch_skip_min) \
    X(BOOL, NONE, rightaltkey) \
    X(BOOL, NONE, use_5casis) \
    X(INT, NONE, x) \
    X(INT, NONE, y) \
    X(INT, NONE, transparent_mode) \
    X(INT, NONE, shading) \
    X(BOOL, NONE, use_alphablend) \
    X(INT, NONE, wallpaper_place) \
    X(INT, NONE, wallpaper_align) \
    X(BOOL, NONE, stop_when_moving) \
    X(BOOL, NONE, use_ddb) \
...
int iso2022_width (struct iso2022_data *this, unsigned int c);
...
#define l10n_translate(str, buf)  l10n_translate_s(str, buf, lenof(buf))
