...
static void xtrans_load_bitmap()
{
    ...
    */
    ...
    if (i > 0 && i < 255) {
        conf_set_bool(conf, CONF_use_alphablend, true);
...
static HICON load_main_icon(HINSTANCE inst)
{
    ...
    if (icon_file->path[0] != '\0') {
        char *buffer = snewn(strlen(icon_file->path) + 1, char);
        char *comma;
        int index = 0;
        strcpy(buffer, icon_file->path);
    ...
    unsigned long attr, int lattr, truecolour truecolour)
{
    ...
    for (remaining = len; remaining > 0;
        ...
        if (ucsdata.dbcs_screenfont && (text[0] & CSET_MASK) == CSET_ACP) {
            ...
            ExtTextOutW2(wintw_hdc, x + xoffset,
                        y - font_height * (lattr == LATTR_BOT) + text_adjust,
                        ETO_CLIPPED | (opaque ? ETO_OPAQUE : 0),
                        &line_box, uni_buf, nlen,
                        lpDx_maybe, !!(attr & ATTR_WIDE));
            if (bold_font_mode == BOLD_SHADOW && (attr & ATTR_BOLD)) {
                SetBkMode(wintw_hdc, TRANSPARENT);
                ExtTextOutW2(wintw_hdc, x + xoffset - 1,
                            y - font_height * (lattr ==
    ...
    unsigned long attr, int lattr, truecolour truecolour)
{
    ...
    if ((attr & TATTR_PASCURS) && (ctype == 0 || term->big_cursor)) {
        ...
        oldpen = SelectObject(wintw_hdc, CreatePen(PS_SOLID, 0, colour));
        ...
        if (attr & TATTR_ACTCURS) {
            HPEN oldpen;
            oldpen =
                SelectObject(wintw_hdc, CreatePen(PS_SOLID, 0, colour));
            ...
            for (i = 0; i < length; i++) {
                if (i % 2 == 0) {
                    SetPixel(wintw_hdc, startx, starty, colour);
                }
                startx += dx;
                starty += dy;
            }
...
#define SHOW_TOASCII_RESULT
    ...
    if (key_down && (conf_get_bool(conf, CONF_rightaltkey) || (keystate[VK_RMENU] & 0x80) == 0) && !compose_state) {
